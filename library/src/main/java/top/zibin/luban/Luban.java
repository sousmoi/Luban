...
public class Luban implements Handler.Callback {
          ...
          @Override public void run() {
            try {
              ...
              File result = Checker.isNeedCompress(mLeastCompressSize, path) ?
                  new YaoHouEngine(path, getImageCacheFile(context, Checker.checkSuffix(path))).compress() :
                  new File(path);
  ...
  @WorkerThread private File get(String path, Context context) throws IOException {
    return new YaoHouEngine(path, getImageCacheFile(context, Checker.checkSuffix(path))).compress();
  }
  ...
  @WorkerThread private List<File> get(Context context) throws IOException {
    ...
    while (iterator.hasNext()) {
      ...
      if (Checker.isImage(path)) {
        results.add(new YaoHouEngine(path, getImageCacheFile(context, Checker.checkSuffix(path))).compress());
      }
      iterator.remove();
    }
